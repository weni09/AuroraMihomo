    ...
    fields: [
      {
        ...
        title: '开启域名嗅探',
        type: 'switch',
        help: '从 TLS/HTTP 流量还原域名，便于 IP 直连也能匹配域名规则。开启后若策略组测速大量超时，优先检查下方「覆盖目标」与 QUIC 嗅探（见各字段说明）。',
      },
      {
        key: 'sniffer.force-dns-mapping',
        title: '强制 DNS 映射',
        type: 'switch',
        help: '对已有 DNS 映射的连接也强制再嗅探。fake-ip 模式较有用；redir-host 下可关以降低干扰。',
      },
      {
        key: 'sniffer.parse-pure-ip',
        title: '解析纯 IP',
        type: 'switch',
        help: '对没有域名的纯 IP 连接尝试嗅探。与「覆盖目标」同开时，容易改写测速/部分直连 IP 流量，出问题可先关掉。',
      },
      {
        key: 'sniffer.override-destination',
        title: '用嗅探结果覆盖连接目标',
        type: 'switch',
        help: '用嗅探到的域名覆盖原始目标再匹配规则。全局 true 时，url-test/面板连通测试（如 gstatic generate_204）与部分节点握手可能被改写而超时。推荐全局 false，仅在 sniffer.sniff 里按协议需要时再开。',
      },
      {
        key: 'sniffer.sniff',
        title: '按协议配置嗅探 (YAML)',
        ...
        codeHeight: '200px',
        placeholder:
          'HTTP:\n  ports: [80, 8080-8880]\nTLS:\n  ports: [443, 8443]\n# 不建议默认嗅探 QUIC/443：易干扰 HTTP/3 与部分测速\n# QUIC:\n#   ports: [443]',
        help: '决定对哪些协议/端口嗅探。只开总开关不配这里则不生效。QUIC 嗅探 443 常导致异常，非必要可省略。',
      },
      {
      ...
      },
      {
        ...
        help: '跳过嗅探。建议加入测速域名（如 www.gstatic.com / +.gstatic.com），避免连通测试被嗅探改写。',
      },
      {

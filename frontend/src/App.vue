...
<template>
  ...
  <div class="min-h-dvh bg-canvas text-fg flex">
    ...
    <aside
      ...
      ref="drawer"
      tabindex="-1"
      class="safe-py safe-pl fixed inset-y-0 left-0 z-40 w-64 shrink-0 bg-surface text-fg border-r border-line flex flex-col shadow-xl transition-transform duration-200 focus:outline-none lg:sticky lg:top-0 lg:inset-y-auto lg:h-dvh lg:z-auto lg:translate-x-0"
      :class="drawerOpen ? 'translate-x-0' : '-translate-x-full'"
    >
      <div class="p-6 border-b border-line shrink-0 flex items-center gap-3">
        <AppLogo :size="32" />
        <h1 class="text-xl font-bold tracking-wider text-primary">AuroraMihomo</h1>
      ...
      <nav class="flex-1 min-h-0 overflow-y-auto p-4 space-y-2 text-sm">
        <RouterLink
      ...
      <div class="p-4 border-t border-line shrink-0 space-y-3">
        <ThemeToggle />
        <Button
          variant="ghost"

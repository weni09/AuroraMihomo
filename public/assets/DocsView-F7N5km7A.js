...
| **域名解析** | DNS 全部行为：enhanced-mode（推荐 fake-ip）、fake-ip 段与过滤、各类 nameserver、respect-rules 等。**用 TUN 时必须开 dns.enable** |\r
...
| **高级参数** | 兜底所有未建模的顶层键：\`listeners\` \`proxy-providers\` \`sub-rules\` \`tls\` \`experimental\` \`hosts\` \`tunnels\` \`ntp\` 等 |\r
\r
**两个必须知道的交互**：\r
\r
...
- **调整系统参数**：往 \`/etc/sysctl.d/99-auroramihomo.conf\` 写本程序需要的内核参数，然后 \`sysctl --system\` 让它立即生效\r
\r

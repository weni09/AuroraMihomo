...
function generateColorScale(baseHex, darkHex, lightHex) {
  const safeBase = baseHex || '#2563EB';
  // Use provided shades or generate approximations
  return {
    "50": { "$value": lightHex || adjustBrightness(safeBase, 0.9), "$type": "color" },
    "100": { "$value": lightHex || adjustBrightness(safeBase, 0.8), "$type": "color" },
    "200": { "$value": adjustBrightness(safeBase, 0.6), "$type": "color" },
    "300": { "$value": adjustBrightness(safeBase, 0.4), "$type": "color" },
    "400": { "$value": adjustBrightness(safeBase, 0.2), "$type": "color" },
    "500": { "$value": safeBase, "$type": "color" },
    "600": { "$value": darkHex || adjustBrightness(safeBase, -0.15), "$type": "color" },
    "700": { "$value": adjustBrightness(safeBase, -0.3), "$type": "color" },
    "800": { "$value": adjustBrightness(safeBase, -0.45), "$type": "color" },
    "900": { "$value": adjustBrightness(safeBase, -0.6), "$type": "color" }
  };
}
...
function adjustBrightness(hex, percent) {
  const safeHex = typeof hex === 'string' && hex.startsWith('#') ? hex : '#2563EB';
...
function main() {
  ...
  console.log(`   Primary: ${colors.primary.name} (${colors.primary.base || '#2563EB'})`);
  console.log(`   Secondary: ${colors.secondary.name} (${colors.secondary.base || '#7C3AED'})`);
  console.log(`   Accent: ${colors.accent.name} (${colors.accent.base || '#10B981'})\n`);

  colors.primary.base = colors.primary.base || '#2563EB';
  colors.secondary.base = colors.secondary.base || '#7C3AED';
  colors.accent.base = colors.accent.base || '#10B981';

...
Design social media images via HTML/CSS rendering + screenshot export. Orchestrates `ui-ux-pro-max`, `brand`, `design-system`, and `agent-browser` skills.
...
Invoke `normal task planning` skill to create persistent TODO tasks via Claude's native task orchestration. Break down into:
...
Present ideas to user via `ask the user directly` for approval before designing.
...
3. **Randomly invoke ONE of:** `ui-ux-pro-max` OR `a local frontend implementation workflow` — for layout, hierarchy, visual balance. Pick one at random each run for design variety.
...
Use Chrome headless, `agent-browser` skill, or Playwright/Puppeteer to capture exact-size screenshots.
...
Use agent-browser or `agent-browser` skill to visually inspect each exported PNG:
...
- AI image generation (use `local image generation workflow` skill for that)

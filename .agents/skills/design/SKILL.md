...
> **⚠️ 本仓库中此 skill 部分可用，且与本项目日常开发无关。**
>
> 可用：`scripts/logo/*`、`scripts/cip/*`、`scripts/icon/generate.py` 的检索功能（实测 `python .agents/skills/design/scripts/logo/search.py "tech logo"` 正常返回），以及 `references/` 下全部文档。图像生成部分需 `GEMINI_API_KEY`。
>
> 不可用：`scripts/search-slides.py`、`scripts/generate-tokens.cjs`、`scripts/validate-tokens.cjs`（依赖未安装的 `design-system` skill）；`scripts/inject-brand-context.cjs`、`scripts/validate-asset.cjs` 实际位于 `brand/scripts/` 且需先创建 `brand/docs/brand-guidelines.md`；`agent-browser` skill 未安装，截图导出需改用本会话的 `browser-use` 插件。
>
> 本 skill 的描述覆盖了 `brand` / `slides` / `banner-design` / `ui-styling` 的能力，四者自动触发时会互相竞争。本仓库是 Mihomo 配置管理平台，这些品牌视觉/演示文稿能力不应在常规前后端任务中触发；前端主题改动请直接编辑 `frontend/src/assets/main.css` 与 `frontend/tailwind.config.js`。
...
After generation, **ALWAYS** ask user about HTML preview via `ask the user directly`. If yes, invoke `/ui-ux-pro-max` for gallery.
...
22 art direction styles across social, ads, web, print. Uses `ui-styling`, `local image generation workflow`, `available multimodal/image tooling`, `agent-browser` skills.

Load `references/banner-sizes-and-styles.md` for complete sizes and styles reference.

### Banner: Workflow

1. **Gather requirements** via `ask the user directly` — purpose, platform, content, brand, style, quantity
2. **Research** — Activate `ui-ux-pro-max`, browse Pinterest for references
3. **Design** — Create HTML/CSS banner with `ui-styling`, generate visuals with `local image generation workflow`/`available multimodal/image tooling`
4. **Export** — Screenshot to PNG at exact dimensions via `agent-browser`
...
Multi-platform social image design: HTML/CSS → screenshot export. Uses `ui-ux-pro-max`, `brand`, `design-system`, `agent-browser` skills.

Load `references/social-photos-design.md` for sizes, templates, best practices.

### Social Photos: Workflow

1. **Orchestrate** — `normal task planning` skill for TODO tasks; parallel subagents for independent work
2. **Analyze** — Parse prompt: subject, platforms, style, brand context, content elements
3. **Ideate** — 3-5 concepts, present via `ask the user directly`
4. **Design** — `brand` → `design-system` → randomly invoke `ui-ux-pro-max` OR `a local frontend implementation workflow`; HTML per idea × size
5. **Export** — `agent-browser` or Playwright screenshot at exact px (2x deviceScaleFactor)
6. **Verify** — Use agent-browser or `agent-browser` skill to visually inspect exported designs; fix layout/styling issues and re-export
...
**Related Skills:** ui-styling, ui-ux-pro-max, available multimodal/image tooling, chrome-devtools

...
func (l *UpdateCheckLogic) UpdateCheck() (resp *types.Result, err error) {
	...
	mihomoCheck, zashCheck := l.svcCtx.Updater.CheckLatest(l.ctx, localVersion)
	...
	// 只有两个组件的版本查询都失败、完全无法判断状态时才算检查失败。
	success := mihomoCheck.Error == "" || zashCheck.Error == ""

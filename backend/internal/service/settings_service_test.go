...
// Update 应持久化 raw 加速源，LoadAndApply 应回灌并推给注册的接收方
func TestSettingsRawCDNPersistAndApply(t *testing.T) {
	svc, db := newTestSettingsService(t)

	// 注册 raw 源推送接收方，验证设置变更后收到清洗后的列表
	var pushed []string
	svc.SetRawCDNFunc(func(p []string) { pushed = append([]string{}, p...) })

	if _, err := svc.Update(UpdateSettingsInput{
		RawCDNProviders: []string{"ghproxy.com", "mirror.ghproxy.com", "ghproxy.com"},
	}); err != nil {
		t.Fatalf("Update 失败: %v", err)
	}
	if len(pushed) == 0 || pushed[0] != "ghproxy.com" {
		t.Fatalf("设置变更后应推送 raw 源, got %#v", pushed)
	}
	got := svc.Get()
	if len(got.RawCDNProviders) == 0 {
		t.Fatal("raw 源应已持久化")
	}

	// 模拟重启：新实例 LoadAndApply 应恢复 raw 源并再次推送
	fresh := NewSettingsService(db, svc.updater)
	var pushed2 []string
	fresh.SetRawCDNFunc(func(p []string) { pushed2 = append([]string{}, p...) })
	if err := fresh.LoadAndApply(); err != nil {
		t.Fatalf("LoadAndApply: %v", err)
	}
	if got := fresh.Get().RawCDNProviders; len(got) == 0 {
		t.Fatal("重启后 raw 源应恢复")
	}
	if len(pushed2) == 0 || pushed2[0] != "ghproxy.com" {
		t.Fatalf("LoadAndApply 应推送 raw 源, got %#v", pushed2)
	}
}

// LoadAndApply 应回灌上次成功的 raw 加速源
func TestLoadAndApplyRestoresLastRawCDNProvider(t *testing.T) {
	svc, db := newTestSettingsService(t)
	if err := db.SetSetting(settingLastRawCDNProvider, "mirror.ghproxy.com"); err != nil {
		t.Fatal(err)
	}
	if err := svc.LoadAndApply(); err != nil {
		t.Fatalf("LoadAndApply: %v", err)
	}
	if got := svc.updater.LastRawCDNProvider(); got != "mirror.ghproxy.com" {
		t.Fatalf("应回灌上次成功 raw 源, 实际 %q", got)
	}
	if got := svc.Get().LastRawCDNProvider; got != "mirror.ghproxy.com" {
		t.Fatalf("Get 应带出 lastRawCdnProvider, 实际 %q", got)
	}
}
